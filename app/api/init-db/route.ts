...
export async function GET() {
  try {
    ...
    if (!existingTables.includes("sign_records")) {
      await sql`
      ...
      `
      console.log("Created sign_records table")
    }
    ...
    const signRecordCount = await sql`SELECT COUNT(*) as count FROM sign_records`
    if (Number.parseInt(signRecordCount[0].count) === 0) {
      await sql`
        ...
        VALUES 
          ...
          ('王五', 8000, 'C3', 'pending')
      `
      console.log("Added sample sign records")
    }
